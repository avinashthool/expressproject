<div class="asg-image {{ $ctrl.asg.classes }}"
	 ng-class="{'modalon' : $ctrl.modalAvailable }"
	...
	<div class="images {{ $ctrl.asg.direction }} {{ $ctrl.asg.options.image.transition }}"
		...
		<div class="img"
			 ng-repeat="(key,file) in $ctrl.asg.files track by $index"
			 ng-show="$ctrl.asg.selected == key"		
			 ng-style="$ctrl.asg.preloadStyle(file, 'image')"
			...
			<div class="source {{ $ctrl.config.size }}"
				 ...
				 ng-click="$ctrl.modalOpen($event)"></div>
